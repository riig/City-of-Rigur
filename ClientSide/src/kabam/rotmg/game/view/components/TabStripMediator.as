...
public class TabStripMediator extends Mediator {
    ...
    private function addTabs(_arg1:Player):void {
        ...
        this.view.addTab(this.iconFactory.makeIconBitmap("lofiObj3", 789), new InventoryTabContent(_arg1));
        if (this.doShowStats) {
            this.view.addTab(this.iconFactory.makeIconBitmap("lofiObj2", 246), new StatsTabContent(this.view.HEIGHT));
        }
        if (_arg1.hasBackpack_) {
            this.view.addTab(this.iconFactory.makeIconBitmap("lofiObj3", 789), new BackpackTabContent(_arg1));
        }
        else {
            this.updateBackpack.add(this.onUpdateBackPack);
        }
        if (this.petModel.getActivePet()) {
            this.view.addTab(this.iconFactory.makeIconBitmap("lofiObj3", 789), new PetsTabContentView());//meh its set to backpack logo cuz cba to find picture for pet tab considering pets arent used
        }
    }
    ...
    private function onUpdateBackPack(_arg1:Boolean):void {
        ...
        if (_arg1) {
            ...
            this.view.addTab(this.iconFactory.makeIconBitmap("lofiObj3", 789), new BackpackTabContent(_local2));
            this.updateBackpack.remove(this.onUpdateBackPack);
        }
    }
