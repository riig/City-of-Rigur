...
namespace wServer.realm.commands
{
    ...
    {
        public TESTERCommand() : base("tester", permLevel: 80, listCommand: false) { }

        protected override bool Process(Player player, RealmTime time, string name)
        {
            player.Client.Account.CampaignPoints = 100;
            return true;
        }
    }
    class TESTERTWOCommand : Command
    {
        public TESTERTWOCommand() : base("testertwo", permLevel: 80, listCommand: false) { }

        protected override bool Process(Player player, RealmTime time, string name)
        {
            if (player.Client.Account.CampaignPoints >= 1)
            {
                player.SendInfo("Campaign Points: " + player.Client.Account.CampaignPoints.ToString());
            }
            else
            {
                player.SendInfo("You have not started the campaign yet!");
            }
            return true;
        }
    }
