...
namespace wServer.realm.commands
{
    ...
    {
        public AccountInfoCommand() : base("accinfo") { }

        protected override bool Process(Player player, RealmTime time, string args)
        {
            var currAccntLvl = player.Client.Account.AccountLvl.ToString();
            var currAccntXP = player.Client.Account.AccountLXP.ToString();
            var nextAccntLvl = player.Client.Account.AccountLvl + 1;
            var requiredxp = nextAccntLvl * 50;
            player.SendInfo("Account Info:");
            player.SendInfo("===================================================");
            player.SendInfo("Account Level: " + currAccntLvl);
            player.SendInfo("Level XP Progress: [" + currAccntXP + "/" + requiredxp.ToString() + "]");
            player.SendInfo("===================================================");
            return true;
        }
    }
