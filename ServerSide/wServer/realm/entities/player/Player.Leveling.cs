...
namespace wServer.realm.entities
{
    public partial class Player
    {
        ...
        public void UpdateAccountXP(Player player, string msg, int amnt) //example: UpdateAccountXP(Client.Player, "Used a Key!", 10)
        {                                                                //this gives player 10 xp and shows them whatever message you wrote explaining why they just got XP

            var currentxp = player.Client.Account.AccountLXP; //current XP amount
            var currentlvl = player.Client.Account.AccountLvl; //current level amount
            var nextlvl = currentlvl + 1; //the level after your current level
            var requiredxp = nextlvl * 50; //xp goal is determined by taking your level + 1 and multiplying by 50 (it basically adds 50 XP to each XP goal as you level)
            var newxp = currentxp + amnt; //the amount of xp you have after you have been rewarded XP
            var xpaway = requiredxp - newxp;
            if (newxp < requiredxp) 
            {
                player.Client.Account.AccountLXP = currentxp + amnt;
                BroadcastSync(new Notification()
                {
                    ObjectId = Id,
                    Color = new ARGB(0xffa500),
                    Message = msg.ToString() + " | +" + amnt.ToString() + " Account XP [" + player.Client.Account.AccountLXP.ToString() + "/" + requiredxp.ToString() + "]",
                });
                BroadcastSync(new Text()
                {
                    Name = "XP Bot",
                    NameColor = 0xffa500,
                    TextColor = 0xffa500,
                    ...
                    BubbleTime = 10,
                    Recipient = "",
                    Txt = "'" + msg.ToString() + "' | +" + amnt.ToString() + "XP | You are " + xpaway.ToString() + "XP away from reaching: Lvl " + nextlvl.ToString()
                });
            }

            else if (newxp >= requiredxp)
            {
                player.Client.Account.AccountLXP = 0; //resets, xp dont get carried
                player.Client.Account.AccountLvl = currentlvl + 1;
                BroadcastSync(new Notification()
                {
                    ObjectId = Id,
                    Color = new ARGB(0xFFFF00),
                    Message = "Account Leveled Up! [Lvl " + player.Client.Account.AccountLvl.ToString() + "]",
                });
                BroadcastSync(new Text()
                {
                    Name = "XP Bot",
                    ...
                    BubbleTime = 10,
                    Recipient = "",
                    Txt = "'" + msg.ToString() + "' | +" + amnt.ToString() + "XP | You have leveled up to: Lvl " + player.Client.Account.AccountLvl.ToString()
                });
            }
        }
